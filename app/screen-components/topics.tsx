...
const TopicsScreen = ({ navigation }) => {
  const [active, setActive] = useState([1, 5, 4]);
  ...
  const categories = [
    ...
    { id: 10, name: "Volby do parlamentu", active: true },
              ...
              <CategoryButton
                ...
                onPress={() => {
                  if (active.includes(category.id)) {
                    const items = active.filter((item) => item !== category.id);
                    setActive(items);
                  } else {
