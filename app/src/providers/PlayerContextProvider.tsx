...
import { usePlayArticleMutation } from "@app/graphql/generated/schema";
...
function logArticlePlayed(articleID: string) {
  console.info("logArticlePlayed(" + articleID + ")");
  const [playArticleMutation, { data, loading, error }] =
    usePlayArticleMutation({
...
const PlayerContextProvider: FC = ({ children }) => {
  ...
  const skipToNext = () => {
    ...
    if (currentTrack != null) {
      const actualArticleIndex = state.queue.indexOf(currentTrack);
