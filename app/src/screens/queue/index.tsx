...
type QueueSection = { key: string; title: string; data: Article[] };

const QueueScreen = ({ navigation }: { navigation: any }) => {
  const theme = useTheme();
  const fonts = useFonts();
  const { state, progress, playArticle, playPause, setPlayed } = usePlayer();
  const [groupedData, setGrouped] = useState<QueueSection[]>([]);

  useEffect(() => {
    var played: QueueSection = {
      key: "0",
      title: "Pokračujte v poslechu",
      data: [],
    };
    var queued: QueueSection = { key: "1", title: "Ve frontě", data: [] };
  ...
  const renderSeparator = ({ section }: { section: any }) => {
    if (section.key == 0) {
      return <View />;
    }
